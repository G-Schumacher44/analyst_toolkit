...
def plot_continuous_distribution(series: pd.Series, save_dir: Path, run_id: str) -> Path:
    ...
    filename = f"{run_id}_dist_cont_{series.name.replace(' ', '_')}.png"
...
def plot_categorical_distribution(series: pd.Series, save_dir: Path, run_id: str, top_n: int = 20) -> Path:
    ...
    save_dir.mkdir(parents=True, exist_ok=True)
    filename = f"{run_id}_dist_cat_{series.name.replace(' ', '_')}.png"
    save_path = save_dir / filename
    plt.savefig(save_path)
    plt.close(fig)
