...
def apply_normalization(df: pd.DataFrame, config: dict) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    try:
        ...
        if rename_map:
            ...
            changelog["renamed_columns"] = pd.DataFrame(
                rename_map.items(), columns=["Original Name", "New Name"]
            )
    ...
    try:
        ...
        if string_cols:
            cleaned_info = [
                {"Column": col, "Operation": "standardize_text"}
            ...
            if cleaned_info:
                changelog["strings_cleaned"] = pd.DataFrame(cleaned_info)
    except Exception as e:
        ...
        if fuzzy_config and fuzzy_config.get("run", False):
            ...
            for col, settings in fuzzy_settings.items():
                if col in df_normalized.columns and df_normalized[col].dtype == "object":
                    ...
                    if not master_list:
                        logging.warning(
                            f"Fuzzy matching for '{col}' skipped: master_list is empty."
                        )
                        continue

                    correction_map = {}
                    vals_to_check = [
                        v for v in df_normalized[col].dropna().unique() if v not in master_list
                    ]
                    for val in vals_to_check:
                        ...
                        if score >= cutoff:
                            ...
                            fuzzy_info.append(
                                {"Column": col, "Original": val, "Corrected": match, "Score": score}
                            )
    ...
    try:
        ...
        if datetime_rules:
            ...
            for col, settings in datetime_rules.items():
                if col in df_normalized.columns:
                    ...
                    multi_formats = (
                        ...
                        if isinstance(settings.get("formats"), list)
                        else None
                    )
                    ...
                    try:
                        ...
                        parsed_series = pd.to_datetime(
                            src,
                            format=None if (multi_formats or fmt in (None, "", "auto")) else fmt,
                            errors="coerce" if errors == "raise" else errors,
                        )
                    ...
                    if errors == "raise":
                        ...
                        if bool(unparsed_mask.any()):
                            ...
                            examples = bad_vals[:5]
                            raise ValueError(
                                f"[strict] Datetime parsing failed for column '{col}': {int(unparsed_mask.sum())} unparsable non-null values. Formats={multi_formats or fmt or 'infer'}. Examples={examples}"
                            )
            ...
            if parsed_info:
                changelog["datetimes_parsed"] = pd.DataFrame(parsed_info)
    except Exception as e:
    ...
    try:
        ...
        if type_map:
            ...
            if coerced_info:
                changelog["types_coerced"] = pd.DataFrame(coerced_info)
    except Exception as e:
