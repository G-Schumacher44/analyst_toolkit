...
def display_normalization_summary(
    changelog: dict, df_original: pd.DataFrame, df_normalized: pd.DataFrame, rules: dict
):
    ...
    def build_action_row(row_keys):
        return "".join(
            [
                ...
                for action in row_keys
                if action in changelog and not changelog[action].empty
            ]
        )

    top_row_html = (
        f'<div style="display: flex; flex-wrap: wrap; gap: 20px;">{build_action_row(top_row)}</div>'
    )
