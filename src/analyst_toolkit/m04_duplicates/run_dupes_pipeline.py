...
def run_duplicates_pipeline(
    ...
    if df is None:
        if "input_path" not in dupes_cfg:
            raise KeyError("Missing 'input_path' in duplicates config.")
    ...
    # In 'remove' mode, the destructive action is performed.
        ...
        if isinstance(checkpoint_cfg, dict) and checkpoint_cfg.get("run", False):
            flagged_path_template = checkpoint_cfg.get(
                "flagged_checkpoint_path", "exports/joblib/{run_id}/{run_id}_m04_df_flagged.joblib"
            )
        ...
        logging.info(
            f"Removed {len(df_original) - len(df_processed)} rows based on 'keep: {dupes_cfg.get('keep', 'first')}' strategy."
        )
    else:
        raise ValueError(
    ...
    duplicates_report = generate_duplicates_report(
        df_original, df_processed, detection_results, mode, df_flagged=df_flagged
    )

    plot_paths = {}
    if settings.get("plotting", {}).get("run", True):
        save_dir = Path(
            settings.get("plotting", {}).get("save_dir", "exports/plots/duplicates/")
        ).joinpath(run_id)
        # Ensure summary_df is valid before plotting
        summary_plot_path = plot_duplication_summary(
            duplicates_report.get("summary"), save_dir, run_id
        )
    ...
    if settings.get("export", False) and duplicates_report:
        export_duplicates_report(report=duplicates_report, config=settings, run_id=run_id)
