...
def display_profile_summary(profile: dict, plot_paths: dict = None, settings: dict = None):
    ...
    """
    if not profile:
        display(HTML("<h4>📊 Data Diagnostics</h4><p><em>Profiling did not run or returned no results.</em></p>"))
        return
    ...
    rows, cols = (shape_df.iloc[0]['Rows'], shape_df.iloc[0]['Columns']) if not shape_df.empty else (0, 0)
    ...
    display(HTML(f"<details><summary><strong>🔬 Quantitative Summary</strong></summary><div style='margin-top: 1em;'><h4>🔢 Descriptive Statistics</h4>{to_html_table(describe_df, full_preview=True)}</div></details>"))
    display(HTML(f"<details><summary><strong>📄 Preview of Duplicated Rows</strong></summary><div style='margin-top: 1em;'>{to_html_table(duplicated_rows_df, max_rows=max_rows)}</div></details>"))
    display(HTML(f"<details><summary><strong>🔍 First Rows Preview</strong></summary><div style='margin-top: 1em;'><h4>📋 First {max_rows} Rows (.head)</h4>{to_html_table(sample_head_df, max_rows=max_rows)}</div></details>"))
