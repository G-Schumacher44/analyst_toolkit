...
async def _toolkit_final_audit(
    ...
    **kwargs
    ...
    # Save to session
    session_id = save_to_session(df_certified, session_id=session_id)
    metadata = get_session_metadata(session_id) or {}
    row_count = metadata.get("row_count")

    # Handle explicit or default export
    export_path = kwargs.get("export_path") or generate_default_export_path(run_id, "final_audit")
    ...
    artifact_url = upload_artifact(artifact_path, run_id, "final_audit", config=kwargs)

    xlsx_path = f"exports/reports/final_audit/{run_id}_FinalAuditReport.xlsx"
    xlsx_url = upload_artifact(xlsx_path, run_id, "final_audit", config=kwargs)
