...
Notebook-friendly HTML dashboard for rendering ETL validation results using IPython display utilities.
...
Used by the ETL module in analyst_toolkit to visualize pipeline hygiene and schema compliance results.
"""
...
def display_validation_summary(results: dict, notebook: bool = True):
    ...
    status_emoji, coverage_pct = ("✅", results.get('summary', {}).get('row_coverage_percent', 'N/A')) if passed_checks == total_checks else ("⚠️", results.get('summary', {}).get('row_coverage_percent', 'N/A'))
    ...
    summary_data = [{"Validation Rule": name.replace('_', ' ').title(), "Description": check['rule_description'], "Status": "✅ Pass" if check['passed'] else f"⚠️ Fail ({len(check['details'])} issues)"} for name, check in checks.items()]
        ...
        if not check['passed']:
            ...
            if name == 'categorical_values':
                for col, violation_info in details.items():
                    ...
                    summary_table = to_html_table(violation_info['invalid_value_summary'], full_preview=True)
                ...
                if name == 'schema_conformity':
                    df_details = pd.DataFrame([{"Issue Type": "Missing", "Columns": ', '.join(details.get('missing_columns',[])) or "None"},{"Issue Type": "Unexpected", "Columns": ', '.join(details.get('unexpected_columns',[])) or "None"}])
                    ...
                    df_details = df_details.rename(columns={'expected': 'Expected Type', 'actual': 'Actual Type'})
    ...
    if drill_down_html:
        display(HTML(f'<div><h3 style="margin-top: 20px;">Failure Details</h3>{drill_down_html}</div>'))