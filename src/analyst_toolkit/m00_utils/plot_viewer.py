...
class PlotViewer:
    ...
    def _build_ui(self):
        ...
        """
        # --- Widget Components ---
        self.title_widget = widgets.HTML(f"<h3 style='margin-top:10px'>{self.title}</h3>" if self.title else "")
        ...
        self.image_output = widgets.Image(
            value=b'',
            format='png',
            layout=widgets.Layout(width='95%', height='auto', object_fit='contain', margin='10px 0 0 0')
        )
        ...
        if initial_category:
            self.plot_dropdown.options = {Path(p).name: p for p in self.plot_paths.get(initial_category, [])}
