...
from analyst_toolkit.m08_visuals.comparison_plots import plot_imputation_comparison, plot_categorical_imputation_comparison
...
def run_imputation_pipeline(config: dict, df: pd.DataFrame = None, notebook: bool = False, run_id: str = None):
    ...
    if settings.get("plotting", {}).get("run", True) and not detailed_changelog.empty:
        save_dir = Path(settings.get("plotting", {}).get("save_dir", "exports/plots/imputation/")) / run_id
        imputed_cols = detailed_changelog['Column'].tolist()
        
        comp_plots = []
        for col in imputed_cols:
            if pd.api.types.is_numeric_dtype(df_imputed[col]):
                plot_path = plot_imputation_comparison(df_original[col], df_imputed[col], save_dir, run_id)
            else:
                plot_path = plot_categorical_imputation_comparison(df_original[col], df_imputed[col], save_dir, run_id)
            if plot_path: comp_plots.append(plot_path)
        if comp_plots:
    ...
    if export_cfg.get("run") and imputation_report:
        export_path = export_cfg.get("export_path", f"exports/reports/imputation/{run_id}_imputation_report.xlsx")
