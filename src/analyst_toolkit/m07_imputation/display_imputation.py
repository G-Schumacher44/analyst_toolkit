...
def display_imputation_summary(report: dict, plot_paths: dict = None):
    ...
    if actions_log_df is None or actions_log_df.empty:
        display(
            HTML(
                "<h4>💉 Imputation Summary</h4><p><em>No imputation actions were performed.</em></p>"
            )
        )
        return
    ...
    # --- 1. Banner ---
    total_filled = (
        int(actions_log_df["Nulls Filled"].sum()) if "Nulls Filled" in actions_log_df.columns else 0
    )
    ...
    if categorical_shift_report:
        ...
        for i, (col, audit_df) in enumerate(categorical_shift_report.items()):
            ...
            norm_vals_df = pd.DataFrame(
                {"Value": vc_after.index, "Count": vc_after.values}
    ...
    if plot_paths:
        ...
        if first_plot_path and os.path.exists(first_plot_path):
            ...
            viewer = PlotViewerOutliers(image_dir=plot_save_dir, title="Imputation Visualizations")
