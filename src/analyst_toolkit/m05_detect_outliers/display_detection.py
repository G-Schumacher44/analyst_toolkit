...
from analyst_toolkit.m00_utils.plot_viewer_comparison import PlotViewerOutliers
...
def display_detection_summary(results: dict, plot_save_dir: str = None):
    ...
    display(
        HTML(
            f"<details open><summary><strong>📋 Outlier Detection Log</strong></summary><div style='margin-top: 1em;'>{to_html_table(outlier_log_df, full_preview=True)}</div></details>"
        )
    )
    if outlier_rows_df is not None and not outlier_rows_df.empty:
        ...
        display(
            HTML(
                f"""<details><summary><strong>🔍 Preview of Rows Containing Outliers</strong></summary><div style="max-height: 400px; overflow-y: auto; margin-top: 1em;">{rows_html}</div></details>"""
            )
        )
    ...
    if (
        plot_save_dir
        and os.path.exists(plot_save_dir)
        and any(f.endswith((".png", ".jpg")) for f in os.listdir(plot_save_dir))
    ):
        viewer = PlotViewerOutliers(image_dir=plot_save_dir, title="Outlier Visualizations")
